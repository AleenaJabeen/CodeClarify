...
const Home = () => {
  ...
  const modeInstructions = {
    ...
    review:
      "Do a thorough code review covering readability, performance, security, and best practices.",
  };
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
        try {
            const res = await fetch("http://127.0.0.1:5000/detect-language", {
                method: "POST",
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        try {
          ...
          setAnalysisResult({
            ...
            concepts: Array.isArray(parsedJson.concepts)
              ? parsedJson.concepts
              : [],
          ...
          console.error(
            "JSON formatting issue, resorting to text preview:",
            jsonParseError,
          );
      ...
      alert(
        "Could not connect to backend server. Make sure your Python Flask app is running.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-[#0e0e10] text-gray-200 font-sans selection:bg-blue-500/30 pb-24">
      ...
      <main className="max-w-3xl mx-auto pt-16 px-6 flex flex-col items-center">
        ...
        <p className="text-gray-500 text-center text-base mb-10 max-w-lg leading-relaxed">
          Paste any Python, JavaScript, or C++ snippet and get a full breakdown
          — logic, concepts, bugs, and improvements.
        </p>
        ...
        <div className="w-full bg-[#18181b] border border-white/10 rounded-2xl shadow-2xl overflow-hidden mb-4">
          ...
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-white/5">
            <span
              className={`text-[11px] font-semibold tracking-wider uppercase px-2.5 py-0.5 rounded-full border bg-white/[0.02]
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-2.5 border-t border-white/5 bg-white/[0.01]">
            <div className="flex gap-1.5 flex-wrap">
              {[
                ...
                { id: "bugs", label: "Bugs only", icon: <Bug size={13} /> },
                {
                  id: "summary",
                  label: "Quick summary",
          ...
          <div className="w-full bg-[#18181b] border border-white/10 rounded-2xl overflow-hidden shadow-2xl mt-4 animate-fadeIn">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/6">
              ...
              <div className="flex gap-1.5">
                <button
                  onClick={handleCopy}
                  className="flex items-center gap-1 border border-white/10 rounded-lg px-2.5 py-1 text-xs text-gray-400 hover:bg-white/5 hover:text-gray-200 transition-all"
                >
                  {copied ? (
            ...
            <div className="flex gap-0.5 px-4 py-2 border-b border-white/5 overflow-x-auto">
              {[
                ...
                {
                  id: "issues",
                  label: `Issues ${analysisResult.issues?.length ? `(${analysisResult.issues.length})` : ""}`,
                  icon: <Bug size={13} />,
                },
                {
                  id: "suggestions",
                  ...
                  <div className="flex flex-col gap-3.5">
                    {analysisResult.steps.map((step, idx) => (
                      <div key={idx} className="flex gap-3 items-start">
                        ...
                        <p className="text-gray-300 text-[13.5px] leading-relaxed pt-0.5">
                          {step}
                        </p>
                      </div>
                    ))}
                ...
                (analysisResult.concepts?.length > 0 ? (
                  <div className="flex flex-col gap-2">
                    {analysisResult.concepts.map((concept, idx) => {
                      ...
                      const title =
                        splitIdx !== -1 ? concept.slice(0, splitIdx) : concept;
                      const desc =
                        splitIdx !== -1 ? concept.slice(splitIdx + 1) : "";
                      return (
                        <div
                          key={idx}
                          className="bg-white/[0.02] border border-white/[0.07] rounded-xl p-3"
                        >
                  ...
                  renderEmptyTab(
                    ...
                    "No specific architecture concepts identified.",
                  )
                ))}
                ...
                (analysisResult.issues?.length > 0 ? (
                  <div className="flex flex-col gap-2">
                    {analysisResult.issues.map((issue, idx) => (
                      <div
                        key={idx}
                        className="bg-red-500/[0.05] border border-red-500/15 rounded-xl p-3 flex gap-2.5 items-start"
                      >
                        <AlertTriangle
                          size={16}
                ...
                (analysisResult.suggestions?.length > 0 ? (
                  <div className="flex flex-col gap-2">
                    {analysisResult.suggestions.map((suggestion, idx) => (
                      <div
                        key={idx}
                        className="bg-green-500/[0.04] border border-green-500/15 rounded-xl p-3 flex gap-2.5 items-start"
                      >
                        <Check
                          size={16}
                  ...
                  renderEmptyTab(
                    <Wand2 size={24} className="text-gray-700" />,
